...
async def get_delivery(
    delivery_id: int, session: AsyncSession = Depends(ConManager.get_session)
):
    """Get delivery by ID"""
    delivery = await DeliveryService.get_by_id(session, delivery_id)
    return delivery
...
async def create(
    ...
    delivery = await DeliveryService.create_delivery(session, data)
    return delivery
